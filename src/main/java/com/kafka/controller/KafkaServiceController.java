...
public class KafkaServiceController {
	...
	protected ResponseEntity<BaseResponse> printTopic() {
		String topic = "messages";
		kafkaService.printTopic(topic);
		return new ResponseEntity<>(BaseResponse.builder().message("Printed").success(true).build(),
				HttpStatus.OK);
	}
}
